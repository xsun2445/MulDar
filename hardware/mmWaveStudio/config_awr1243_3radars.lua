...
ADC_SAMPLE_RATE =       2800        -- ksps
RAMP_END_TIME =         100.0      -- us
-- Chirp Profile Config
-- IDEL_TIME =             15.0
-- FREQ_SLOPE =            38.816929   -- MHz/us
-- ADC_START_TIME =        7           -- us
...
-- Int32 ar1.FrameConfig(UInt16 chirpStartIdx, UInt16 chirpEndIdx, UInt16 frameCount, UInt16 loopCount, Single periodicity, Single triggerDelay, Byte numDummyChirp, UInt16 TriggerSelect) - Frame Configuration API defines Frame formation which has sequence of chirps to be transmitted subsequently
-- _I_ UInt16	chirpStartIdx	 - First Chirp Start Index number
-- _I_ UInt16	chirpEndIdx	 - Last chirp Index number
-- _I_ UInt16	frameCount	 - Number of frames to transmit
-- _I_ UInt16	loopCount	 - Number of times to repeat from start chirp to last chirp in each frame
-- _I_ Single	periodicity	 - Each frame repetition period in ms
-- _I_ Single	triggerDelay	 -  Optional time delay from sync in trigger to the occurrence of frame chirps in µs
-- _I_ Byte	numDummyChirp	 - No. of chirps in a frame to skip (at the end of the frame) for HSI streaming
-- _I_ UInt16	TriggerSelect	 - TriggerSelect

if (0 == ar1.FrameConfig(0, num_chirps, NUM_FRAMES, num_loop_chirp, t_frame, 0, 0, 2)) then
